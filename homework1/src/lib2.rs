...
fn split_clone(s: &str) -> Vec<String> {
    s.split(' ').map(|ss| ss.to_string()).collect()
}
...
fn pick_longest<T: Into<String>>(s1: T, s2: T) -> String {
    let string1 = s1.into();
    let string2 = s2.into();
...
// the case?
// &str is the reference which is owner dies out when the function return
...
// original call in add1_test(): add1(x)
// fn add1(mut x: i32) -> () { // x is moved here, which means x is dropped if return
//    x += 1;                  // pass the reference in, and modify it
// }
